terraform {
//  source = "github.com/geometry-labs/terraform-aws-k8s-cluster.git?ref=${local.vars.versions.k8s-cluster}"
  source = "source"
}
...
inputs = {
  ...
  subnet_ids = local.vars.create_network ? dependency.network.outputs.public_subnets : local.vars.k8s_subnets
